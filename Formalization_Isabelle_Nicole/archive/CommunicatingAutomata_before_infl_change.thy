...
(*NEED TO FIX: add projection to p and q, otherwise this is only correct if each node sends 
...
lemma child_matching_word_impl_in_infl_lang:
  assumes "tree_topology" and "is_parent_of p q" and "w \<in> \<L>(q)" and "is_in_infl_lang q w" and  "((w'\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = ((w\<down>\<^sub>!)\<down>\<^sub>!\<^sub>?)" and "w' \<in> \<L>(p)"
...
lemma infl_parent_child_matching_ws2 :
  fixes w :: "('information, 'peer) action word"
  assumes "w \<in> \<L>\<^sup>*(q)" and "is_parent_of p q" and "((w'\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = ((w\<down>\<^sub>!)\<down>\<^sub>!\<^sub>?)" and "w' \<in> \<L>(p)"
  shows "w' \<in> \<L>\<^sup>*(p)"
...
lemma
  assumes "((w'\<down>\<^sub>q))\<down>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>} \<in> ((\<L>\<^sup>*(q))\<downharpoonright>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>})" and "w'\<down>\<^sub>p = \<epsilon>" and "w'\<down>\<^sub>! \<in> \<L>\<^sub>\<zero>" and "is_parent_of p q"
  shows "True"
  sorry

(*this one might be unnecessary but the conclusion of the lemma under this is needed*)
lemma subword_of_sync_is_receivable:
  assumes "w'\<down>\<^sub>! \<in> \<L>\<^sub>\<zero>" and "w'\<down>\<^sub>p = \<epsilon>" and "((w'\<down>\<^sub>q)\<down>\<^sub>!)\<down>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>} \<in> ((\<L>\<^sub>!\<^sup>*(q))\<downharpoonright>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>})" and "is_parent_of p q"
