...
next
  ...
  then have "w\<down>\<^sub>q = w"  using w_in_peer_lang_impl_p_actor by auto
      (* prepend first word to wn \<rightarrow>   w(n-1) \<sqdot> w(n) *)
      (*then have "concat_infl q w (p # ps) (w' \<sqdot> w)" *)
      (*obtain w1 \<sqdot> w2 \<sqdots> wn*)
...
next
  ...
  then obtain v a where w_def: "w = v \<sqdot> [a]" and v_len: "length v = n" by (metis length_Suc_conv_rev)
  ...
  have "(v \<sqdot> [a]) = (v \<sqdot> [a])\<down>\<^sub>!" using Suc.prems(1) w_def by fastforce
  ...
  have w_sends_0: "w = ((?v') \<sqdot> [a])\<down>\<^sub>!" by (metis \<open>v \<sqdot> a # \<epsilon> = (v \<sqdot> a # \<epsilon>)\<down>\<^sub>!\<close> adding_recvs_keeps_send_order filter_append w_def)
  then have w_sends_1: "w = (?v')\<down>\<^sub>! \<sqdot> [a]" using \<open>v \<in> \<T>\<^bsub>None\<^esub>\<downharpoonright>\<^sub>!\<close> adding_recvs_keeps_send_order w_def by fastforce
  ...
  proof (cases "is_root q")
    case True
    then have "(v\<down>\<^sub>q \<sqdot> [!\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>]) \<in> (\<L>\<^sup>*(q))" using mbox_trace_to_root_word[of v i q p] using Suc.prems(1) a_def w_def by fastforce
    ...
    then have "?v'\<down>\<^sub>q \<sqdot> [a] \<in> \<L>\<^sup>* q" by (metis (no_types, lifting) \<open>v\<down>\<^sub>q \<sqdot> !\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle> # \<epsilon> \<in> \<L>\<^sup>* q\<close> \<open>w = add_matching_recvs v\<down>\<^sub>! \<sqdot> a # \<epsilon>\<close> a_def
  ...
  next
    ...
    have wq_v'_sends: "wq\<down>\<^sub>! = ((?v')\<down>\<^sub>! \<sqdot> [a])\<down>\<^sub>q" using \<open>w = add_matching_recvs v\<down>\<^sub>! \<sqdot> a # \<epsilon>\<close> \<open>wq\<down>\<^sub>! = w\<down>\<^sub>q\<close> by fastforce
    have v'a3: "((?v')\<down>\<^sub>! \<sqdot> [a])\<down>\<^sub>q = ((?v')\<down>\<^sub>!)\<down>\<^sub>q \<sqdot> [a]\<down>\<^sub>q" by simp
    ...
    have e_trace2: "(e\<down>\<^sub>!) = ((?v' \<sqdot> [a])\<down>\<^sub>!)" using \<open>w = (add_matching_recvs v \<sqdot> a # \<epsilon>)\<down>\<^sub>!\<close> e_trace by blast
    then have "prefix (((wq)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) (((((?v' \<sqdot> [a])\<down>\<^sub>r)\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)"  by (metis (no_types, lifting) e_pref filter_pair_commutative
          wq_def)
    have "((((?v' \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = ((((?v')\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) \<sqdot> (((([a])\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)" by simp
    ...
    then have "((((?v' \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = ((((?v')\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)" by simp
    have "((((?v' \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = (((e\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)" using \<open>e\<down>\<^sub>! = (add_matching_recvs v \<sqdot> a # \<epsilon>)\<down>\<^sub>!\<close> by presburger
    then have "(((e\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = ((((?v')\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)"  using \<open>(add_matching_recvs v \<sqdot> a # \<epsilon>)\<down>\<^sub>!\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>}\<down>\<^sub>!\<^sub>? = add_matching_recvs v\<down>\<^sub>!\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>}\<down>\<^sub>!\<^sub>?\<close>
      by argo
    have v'_match: "(((((?v')\<down>\<^sub>!)\<down>\<^sub>r)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = ((((?v')\<down>\<^sub>?)\<down>\<^sub>q)\<down>\<^sub>!\<^sub>?)" using \<open>w = add_matching_recvs v\<down>\<^sub>! \<sqdot> a # \<epsilon>\<close> v'_recvs_match w_def by force
    then have e_v'_match:"((((e\<down>\<^sub>!)\<down>\<^sub>r)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = ((((?v')\<down>\<^sub>?)\<down>\<^sub>q)\<down>\<^sub>!\<^sub>?)"  using \<open>(a # \<epsilon>)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>} = \<epsilon>\<close> \<open>w = add_matching_recvs v\<down>\<^sub>! \<sqdot> a # \<epsilon>\<close> e_trace by force
    ...
    have "(((((?v' \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>r)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = ((((?v' \<sqdot> [a])\<down>\<^sub>?)\<down>\<^sub>q)\<down>\<^sub>!\<^sub>?)" by (metis (no_types, lifting) e_trace2 e_v'_match filter_pair_commutative v'_q_recvs_inv_to_a)
    have "prefix (wq\<down>\<^sub>?) (((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?)" using v'_q_recvs_inv_to_a wq_recvs_prefix by presburger(*recvs in wq must have same order as in v', otherwise diff trace!*)
    ...
    then obtain wr' x' where v'r_def: "((?v')\<down>\<^sub>r) = wr' \<sqdot> x'" and wr'_match: "(((wr'\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = (((wq)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?)" and "wr' \<in> \<L>\<^sup>* r" by (meson prefixE)
    have "((?v')\<down>\<^sub>r) \<in> \<L>\<^sup>* r" using mbox_exec_to_infl_peer_word[of "?v'" r]  using v_IH by blast
    then have "wr' \<sqdot> x' \<in> \<L>\<^sup>* r" by (simp add: v'r_def)
    ...
    have "\<exists>x. (wq \<sqdot> x) \<in> \<L>\<^sup>* q \<and> (((wq \<sqdot> x)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = ((((wr' \<sqdot> x')\<down>\<^sub>!)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)" using subset_cond_from_child_prefix_and_parent[
    ...
    then have "wq \<sqdot> xs \<sqdot> ys \<in> \<L>\<^sup>* q" by (metis UNIV_def \<open>is_parent_of q r\<close> \<open>wq \<sqdot> x \<in> \<L>\<^sup>* q\<close> assms(3) input_shuffle_implies_shuffled_lang
    ...
    have wqx_match_v'a: "((((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = (((wq \<sqdot> x)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?)"  using e_trace2 e_v'_match v'_proj_inv v'_q_recvs_inv_to_a wqx_match_v' by presburger
    ...
    then have xs_recvs: "(((wq \<sqdot> xs)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = (((((?v' \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>r)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)" using wqx_match_v' wqx_match_v'a by argo
    have v'_eq: "(((((?v' \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>r)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?) = (((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?" using e_trace2 e_v'_match v'_proj_inv v'_q_recvs_inv_to_a by presburger
    then have "(((wq \<sqdot> xs)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = (((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?" using xs_recvs by presburger
    then have "(wq \<sqdot> xs)\<down>\<^sub>? = (((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?)"  using no_sign_recv_prefix_to_sign_inv[of "(wq \<sqdot> xs)\<down>\<^sub>?" "(((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?)"] by (metis filter_recursion no_sign_recv_prefix_to_sign_inv prefix_order.dual_order.antisym
    ...
    then have wqxs_order:"(wq \<sqdot> xs)\<down>\<^sub>? = (((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>?) \<and> (wq \<sqdot> xs)\<down>\<^sub>! = ((?v' \<sqdot> [a])\<down>\<^sub>q)\<down>\<^sub>!" using \<open>(a # \<epsilon>)\<down>\<^sub>q = a # \<epsilon>\<close> \<open>(wq \<sqdot> xs)\<down>\<^sub>! = wq\<down>\<^sub>!\<close> w_sends_0 w_sends_1 wq_to_v'a_trace by force (*same sends & recvs in isolation, respectively*)
    have wqxs_trace_match: "(((wq \<sqdot> xs)\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = (((((v \<sqdot> [a])\<down>\<^sub>!)\<down>\<^sub>r)\<down>\<^sub>{\<^sub>q\<^sub>,\<^sub>r\<^sub>})\<down>\<^sub>!\<^sub>?)"  using \<open>v \<sqdot> a # \<epsilon> = (v \<sqdot> a # \<epsilon>)\<down>\<^sub>!\<close> e_trace e_trace2 w_def xs_recvs by presburger
    let ?wq = "wq \<sqdot> xs"
    show ?thesis using wqxs_order 
    proof (cases "(?v'\<down>\<^sub>q \<sqdot> [a]) \<squnion>\<squnion>\<^sub>? (?wq)")
      case True
      then have "(?v'\<down>\<^sub>q \<sqdot> [a]) \<in> (\<L>\<^sup>*\<^sub>\<squnion>\<^sub>\<squnion>(q))" using input_shuffle_implies_shuffled_lang wqxs_L by blast
      then have "(?v'\<down>\<^sub>q \<sqdot> [a]) \<in> (\<L>\<^sup>*(q))" using theorem_right_qr by simp
...
\<and> e \<in> \<T>\<^bsub>None\<^esub> \<and> ?v' \<in> \<T>\<^bsub>None\<^esub> \<and> (v \<sqdot> [a]) \<in> \<T>\<^bsub>None\<^esub>\<downharpoonright>\<^sub>! \<and> ?v' = (add_matching_recvs v) \<and> ?v'\<down>\<^sub>q \<in> \<L>\<^sup>* q
\<and> ?wq \<in> \<L>\<^sup>* q" 
        by (metis (no_types, lifting) False Suc.prems(1) \<open>(a # \<epsilon>)\<down>\<^sub>q = a # \<epsilon>\<close> \<open>(wq \<sqdot> xs)\<down>\<^sub>! = wq\<down>\<^sub>!\<close>
            \<open>((wq \<sqdot> xs)\<down>\<^sub>?) = ((add_matching_recvs v \<sqdot> a # \<epsilon>)\<down>\<^sub>q)\<down>\<^sub>?\<close> \<open>add_matching_recvs v\<down>\<^sub>q \<sqdot> a # \<epsilon> \<noteq> wq \<sqdot> xs\<close>
...
(*since q can send all its outputs in v and then a, its child p must be able to receive a after its current word in v'*)
  ...
  have "(?wp \<sqdot> [?\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>]) \<in> \<L>\<^sup>* p \<and> (((?wp \<sqdot> [?\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>])\<down>\<^sub>?)\<down>\<^sub>!\<^sub>?) = ((((?wq_full)\<down>\<^sub>!)\<down>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>})\<down>\<^sub>!\<^sub>?)"
    using subset_cond_from_child_prefix_and_parent_act[of p q "?wp" "(?v'\<down>\<^sub>q)" i] by (smt (verit, ccfv_SIG) filter_pair_commutative pq q_full theorem_right_pq v'_recvs_match_pq2
        v'p_in_L)
  then have "(((?v')\<down>\<^sub>p \<sqdot> [(?\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>)])) \<in> \<L>\<^sup>* p" by simp
  ...
  then have a_ok0: "(?v' \<sqdot> ([!\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>] \<sqdot> [?\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>])) \<in> \<T>\<^bsub>None\<^esub>" 
    using mbox_exec_recv_append[of "?v'" i q p] using a_def a_send_ok by (metis (no_types, lifting) append1_eq_conv append_assoc filter_pair_commutative pq v'_recvs_match_pq w_def
        w_sends_1)
  have a_match: "(add_matching_recvs [a]) = ([!\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>] \<sqdot> [?\<langle>(i\<^bsup>q\<rightarrow>p\<^esup>)\<rangle>])"  using a_def by force
  then have a_ok: "((add_matching_recvs v) \<sqdot> (add_matching_recvs [a])) \<in> \<T>\<^bsub>None\<^esub>" using a_ok0 by auto
    ...
    proof (rule conjI)
      ...
      proof auto
        ...
        proof (cases "is_root q")
          ...
          have "\<exists>wa. x'\<down>\<^sub>!\<down>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>}\<down>\<^sub>!\<^sub>? =  wa\<down>\<^sub>!\<^sub>? \<and>  (\<exists>x. wa = x\<down>\<^sub>? \<and> is_in_infl_lang p (w \<sqdot> x))"  using \<open>w \<sqdot> x \<in> \<L>\<^sup>* p\<close> \<open>w'\<down>\<^sub>!\<down>\<^sub>{\<^sub>p\<^sub>,\<^sub>q\<^sub>}\<down>\<^sub>!\<^sub>? = w\<down>\<^sub>?\<down>\<^sub>!\<^sub>?\<close> w'x'_wx_match by auto
