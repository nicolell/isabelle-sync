...
inductive word_in_EffectiveLanguage where
  :: ""

inductive_set
  EffectiveLanguageSend :: "'peer \<Rightarrow> ('information, 'peer) action language"  ("\<L>\<^sub>!\<^sup>* _" [90] 100) and
  EffectiveLanguageRecv :: "'peer \<Rightarrow> ('information, 'peer) action language"  ("\<L>\<^sub>?\<^sup>* _" [90] 100)
  for p :: "'peer" where
ELSRoot: "\<lbrakk>\<P>\<^sub>?(p) = {}; w \<in> \<L>(p)\<rbrakk> \<Longrightarrow> w \<in> \<L>\<^sub>!\<^sup>*(p)" |
ELRRoot: "\<lbrakk>\<P>\<^sub>?(p) = {}; w \<in> \<L>(p)\<rbrakk> \<Longrightarrow> \<epsilon> \<in> \<L>\<^sub>?\<^sup>*(p)" |
ELSNode: "\<lbrakk>\<P>\<^sub>?(p) = {q}; w \<in> \<L>(p); w\<down>\<^sub>? \<in> \<L>\<^sub>?\<^sup>*(p)\<rbrakk> \<Longrightarrow> w\<down>\<^sub>! \<in> \<L>\<^sub>!\<^sup>*(p)" |
ELRNode: "\<lbrakk>\<P>\<^sub>?(p) = {q}; w \<in> \<L>\<^sub>?(p); w\<down>\<^sub>!\<^sub>? \<in> (\<L>\<^sub>!\<^sup>*(q))\<downharpoonright>\<^sub>!\<^sub>?\<rbrakk> \<Longrightarrow> w \<in> \<L>\<^sub>?\<^sup>*(p)"

theorem synchronisability_for_trees:
  assumes "tree_topology"
  shows "is_synchronisable \<longleftrightarrow> (\<forall>p. {?} \<subseteq> {?})"
  sorry
*)
