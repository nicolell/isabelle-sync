...
next
  ...
  next
    ...
    then have "\<exists> ysa. ysa = a # ys" using Cons.IH by blast
    then obtain ysa where "ysa = a # ys" by blast
    then have "x \<notin> elems ysa" by (simp add: \<open>x \<notin> elems ys\<close> \<open>x \<noteq> a\<close>)
    then have "a # xs = ysa @ x # zs" "x \<notin> elems ysa" by (simp_all add: \<open>xs = ys @ x # zs\<close> \<open>ysa = a # ys\<close>)
...
text " balanced n w <=> S (a^n @ w) ===> this means w is either b^n or a^mb^(n+m) (otherwise false)"
fun balanced :: "nat \<Rightarrow> alpha list \<Rightarrow> bool" where
"balanced 0 xs = S xs" |
"balanced n [] = False" |
"balanced (Suc n) xs = (if last xs = b then balanced n (butlast xs) else False)" 


value "take 2 [a,a,b,b]"
value "drop 2 [a,a,b,b, b]"
value "butlast [a,a,a,a,b]"
value "replicate 2 a"
...
lemma "S w \<Longrightarrow> balanced 0 w"
  apply (induction w rule: S.induct)
  apply(auto)
  done

lemma "balanced 0 w \<Longrightarrow> S w"
  by auto

corollary "balanced n w \<longleftrightarrow> S (replicate n a @ w)" (is "?L \<longleftrightarrow> ?R")
proof - 
  ...
  proof (induction rule: balanced.induct)
    case (1 xs)
    then show ?case using "1.prems" balanced.cases by sledgehammer
  next
    case (2 v)
