...
fun hel :: "exp \<Rightarrow> int" where
"hel Var = 1" |
"hel (Const m) = 0" |
"hel (Add t1 t2) = 0" |
...
"s_add x y = (if x = y 
then (Mult (Const 2) x) 
else (if x = (Const 0) then y else (if y = (Const 0) then x else
(Add x y))))"

fun s_mult :: "exp \<Rightarrow> exp \<Rightarrow> exp" where
"s_mult (Const m) (Const n) = Const (m * n)" |
"s_mult (Const m) (Mult (Const n) x) = Mult (Const (m * n)) x" |
"s_mult (Const m) n = (if m = 1 then n 
  else 
    (if m = 0 
      then (Const 0) 
      else (Mult (Const m) n)))" |
...
value "s_mult (Const 1) (Mult (Const 2) Var)"
value "s_mult (Const 2) (Mult (Const 2) Var)"
value "s_add (Mult (Const 2) Var) (Mult (Const 2) Var)"

fun simp_ex :: "exp \<Rightarrow> exp" where
"simp_ex (Const m) = Const m" |
"simp_ex Var = Var" |
"simp_ex (Mult e1 e2) = s_mult (simp_ex e1) (simp_ex e2)"|
"simp_ex (Add e1 e2) = s_add (simp_ex e1) (simp_ex e2)"

value "simp_ex (simp_ex (Add (Mult (Const 2) Var) (Mult (Const 2) Var)))"
...
fun total_occs :: "exp \<Rightarrow> (int \<times> int) list" where
"total_occs (Const m) = [(0, m)]" |
"total_occs Var = [(1,1)]" |
"total_occs (Add e1 e2) = (total_occs e1) @ (total_occs e2)" |
"total_occs (Mult (Const m) e2) = [(hel e2, m)]" |
"total_occs (Mult Var Var) = [(2,1)]" |
"total_occs (Mult e1 e2) = [(hel e1, hel e2)]"

fun sorted_occs ::  "exp \<Rightarrow> (int \<times> int) list" where
"sorted_occs e = sort_key fst (total_occs e)"

fun fill :: "int \<Rightarrow> int \<Rightarrow> (int \<times> int) list" where 
"fill m n = (if n \<le> 0 then (if m > n then [] else [(0,0)]) else (m, 0) # (fill (m+1) (n-1)))"

value "fill 0 2"

fun fill_occs :: "(int \<times> int) list \<Rightarrow> (int \<times> int) list" where
"fill_occs Nil = Nil"|
"fill_occs ((p1, o1) # (p2, o2) # xs) = [(p1, o1)] @ (fill (p1 + 1) (p2 - p1 - 1)) @ [(p2, o2)] @ (fill_occs xs)"|
"fill_occs [(p1, o1)] = (if p1 = 0 then [] else (fill 0 (p1))) @ [(p1, o1)]"

value "fill_occs (sorted_occs Var)"
value "fill_occs (sorted_occs (Const (-1)))"
value "fill_occs [(0,2)]"

fun seg :: " (int \<times> int) list \<Rightarrow> int list" where
"seg [] = []" |
"seg (x # xs) = (snd x) # (seg xs)"

value "hel  (Mult (Const 1) Var)"
value "var_occs (Const 2)"
value "var_occs (Add (Const 3) (Add (Mult (Const 1) Var) (Mult (Const 2) (Mult Var Var))))"
value "simp_ex (Add (Const 3) (Add (Mult (Const 1) Var) (Mult (Const 2) (Mult Var Var))))"
value "total_occs (simp_ex (Add (Const 3) (Add (Mult (Const 1) Var) (Mult (Const 2) (Mult Var Var)))))"
value "total_occs ((Mult (Const 2) (Mult Var Var)))"
value "total_occs (Mult (Const (-1)) Var)"
value "sorted_occs (simp_ex (Add (Const 3) (Add (Mult (Const 1) Var) (Mult (Const 2) (Mult Var Var)))))"


fun coeffs :: "exp \<Rightarrow> int list" where
"coeffs e = seg (fill_occs (sorted_occs (simp_ex e)))"
(* your definition/proof here *)

value "coeffs (Add (Const 3) (Add (Mult (Const 1) Var) (Mult (Const 2) (Mult Var Var))))"
value "coeffs Var"
value "coeffs (Const (-1))"
value "coeffs (Add Var (Const 0))"
value "sorted_occs (Add Var (Const (- 1))) "
value "fill_occs [(0, - 1), (1, 1)]"
value "fill_occs [(2, 1)]"
value "simp_ex (Add Var (Add Var Var))"
