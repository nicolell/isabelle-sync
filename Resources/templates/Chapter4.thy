...
"set Tip  = {}" |
"set (Node l i r) = (set l) \<union> (set r) \<union> {i}"
(* your definition/proof here *)

fun ord :: "int tree \<Rightarrow> bool"  where
"ord Tip = True" |
"ord (Node l i r) = (ord l \<and> ord r \<and> (\<forall>j \<in> set l. j < i) \<and> (\<forall>j \<in> set r. j > i))"
text "ord (Node l i r) = ((ord l) \<and> (ord r) \<and> (card ({j |j. j \<in> (set l) \<and> j > i}::(int set)) = 0) \<and> (card ({j |j. j \<in> (set r) \<and> j < i}::(int set)) = 0) )"
(* your definition/proof here *)

value "set (Tip::(int tree))"
value "set (Node Tip (1::int) (Node Tip 2 Tip))"
value "card ({}:: (int set))"
text "card {j |j. j \<in> (set (Node Tip (1::int) (Node Tip 2 Tip))) \<and> ((j::int) = 2)}"
text "ord  (Node Tip (1::int) (Node Tip 2 Tip))"
...
"ins i (Node l j r) = (if i = j then (Node l j r) else (if i < j then (Node (ins i l) j r) else (Node l j (ins i r)))) " 
(* your definition/proof here *)
...
value "ins 3 (Node Tip 1 (Node (Node Tip 4 Tip) 5 Tip))"
...
P_0 : "palindrome []" |
P_1 : "palindrome [a]" |
P_gen : "palindrome xs \<Longrightarrow> palindrome (a # xs @ [a])"
...
declare palindrome.intros [intro, simp]
...
iter_star : "star r x y \<Longrightarrow> iter r n x y"
text "iter_suc : r x y \<Longrightarrow> iter r n y z \<Longrightarrow> iter r (Suc n) x z"
(* your definition/proof here *)

declare iter.intros [simp, intro]

text\<open>
Correct and prove the following claim:
\<close>

lemma "star r x y \<Longrightarrow> iter r n x y" by auto
(* your definition/proof here *)
...
S_emp : "S []" |
S_app : "S xs \<Longrightarrow> S ys \<Longrightarrow> S (xs @ ys)" |
S_gen : "S xs \<Longrightarrow> S (a # xs @ [b])"
(* your definition/proof here *)

inductive T :: "alpha list \<Rightarrow> bool" where
T_emp : "T []" |
T_gen : "T xs \<Longrightarrow> T ys \<Longrightarrow> T (xs @ (a # ys @ [b]))"
...
lemma TS: "T w \<Longrightarrow> S w"
 apply (induction rule: T.induct)
  apply (simp add: S_emp)
  apply (metis S_app S_gen)
  done
(* your definition/proof here *)

lemma T_gen2 [simp]: "S xs \<Longrightarrow> T xs \<Longrightarrow> T (a # xs @ [b])" by (metis T_emp T_gen eq_Nil_appendI)

value "[] @ [a]"

lemma T_app_Nil [simp]: "T xs \<Longrightarrow> T ( [] @ xs) \<and> T (xs @ [])" by simp


lemma T_app [simp]: "T xs \<Longrightarrow> T ys \<Longrightarrow> T (xs @ ys)"
  sorry
...
aval_N : "aval_rel (N n) _ n" |
aval_V : "aval_rel (V v) s (s v)" |
aval_Plus : "\<lbrakk>aval_rel a1 s e1; aval_rel b1 s e2\<rbrakk> \<Longrightarrow> aval_rel (Plus a1 b1) s (e1 + e2)"
(* your definition/proof here *)

declare aval_rel.intros [simp, intro]

lemma aval_rel_aval: "aval_rel a1 s v \<Longrightarrow> aval a1 s = v"
(* your definition/proof here *)
  apply(induction rule: aval_rel.induct)
  apply(auto)
  done

lemma aval_aval_rel: "aval a1 s = v \<Longrightarrow> aval_rel a1 s v"
  apply(induction a1 arbitrary: s v)
  apply(auto split: aexp.split)
  done
(* your definition/proof here *)

corollary "aval_rel a1 s v \<longleftrightarrow> aval a1 s = v"
  using aval_rel_aval aval_aval_rel by blast
(* your definition/proof here *)
...
ok_emp : "ok n [] n" |
ok_LOADI : "ok n [LOADI x] (Suc n)" |
ok_LOAD : "ok n [LOAD x] (Suc n)" |
ok_ADD : "ok (Suc (Suc n)) [ADD] (Suc n)" |
ok_app : "\<lbrakk>ok m is n'; ok n [i] m\<rbrakk> \<Longrightarrow> ok n (i # is) n'"
(* your definition/proof here *)
...
lemma "ok 0 [LOAD x] (Suc 0)" by simp
(* your definition/proof here *)
...
lemma "ok (Suc (Suc 0)) [LOAD x, ADD, ADD, LOAD y] (Suc (Suc 0))" 
  apply (blast dest:  ok_app) 
  done
(* your definition/proof here *)

text \<open> Prove that @{text ok} correctly computes the final stack size: \<close>

lemma exex1_len [simp]: "length stk = Suc (Suc n) \<Longrightarrow> length (exec1 ADD s stk) = Suc n"
  by (smt (verit, best) Suc_length_conv exec1.simps(3))
...
lemma ok_app_gen [simp]: "\<lbrakk>ok m is2 n'; ok n is1 m\<rbrakk> \<Longrightarrow> ok n (is1 @ is2) n'"
  apply(induction rule: ok.induct)
      apply(auto)
  done

lemma "ok n (comp a1) (Suc n)"
  apply(induction rule: comp.induct)
  apply(simp_all add: ok.intros)
  apply(blast)
  apply(rule ok_app)
  done
